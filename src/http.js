...
module.exports = async client => {
	...
	// Health check endpoints for deployment probes (autoscale probes /, VM uses /healthz)
	fastify.get('/healthz', async (req, res) => res.code(200).send({ status: 'ok' }));
	fastify.get('/', async (req, res) => res.code(200).send({ status: 'ok' }));
